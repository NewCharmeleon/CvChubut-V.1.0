...
@include('carrera.partials.show')
 <div class="row">
        <div class="form-group   {{ ( $errors->has('materias_aprobadas') )?   'has-error' : ''}}   ">  
            ...
            <div class="col-sm-6">
                <input type="number" min="0" max="{{$carrera->cantidad_materias}}" class="form-control" name="materias_aprobadas" id="materias_aprobadas" placeholder="Cantidad" value="{{ (Session::has('errors')) ? old('materias_aprobadas', '') : ( isset($carrera)? $carrera->materias_aprobadas : null ) }}" >
